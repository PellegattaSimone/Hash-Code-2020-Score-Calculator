...
int main()
{
	...
	//opening files
	std::clog << "Opening files..." << std::endl;

	std::ifstream sub, in;

	try
	{
		sub.open("a_example.txt", std::ios_base::in);
		if (!sub)
		{
			sub.open("b_read_on.txt", std::ios_base::in);
			if (!sub)
			{
				sub.open("c_incunabula.txt", std::ios_base::in);
				if (!sub)
				{
					sub.open("d_tough_choices.txt", std::ios_base::in);
					if (!sub)
					{
						sub.open("e_so_many_books.txt", std::ios_base::in);
						if (!sub)
						{
							sub.open("f_libraries_of_the_world.txt", std::ios_base::in);

							if (sub) in.open("data/f_libraries_of_the_world.hashcode", std::ios_base::in);
						}
						else in.open("data/e_so_many_books.hashcode", std::ios_base::in);
					}
					else in.open("data/d_tough_choices.hashcode", std::ios_base::in);
				}
				else in.open("data/c_incunabula.hashcode", std::ios_base::in);
			}
			else in.open("data/b_read_on.hashcode", std::ios_base::in);
		}
		else in.open("data/a_example.hashcode", std::ios_base::in);
